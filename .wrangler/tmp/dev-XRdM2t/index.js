...
// .wrangler/tmp/bundle-4DkXLs/strip-cf-connecting-ip-header.js
...
var CanvasDO = class {
  ...
  async loadCanvas() {
    if (this.canvas)
      return this.canvas;
    const stored = await this.state.storage.get("canvas");
    if (stored) {
      this.canvas = stored;
    } else {
      this.canvas = {
  ...
  async fetch(request) {
    ...
    if (request.headers.get("Upgrade") === "websocket") {
      ...
      const canvas2 = await this.loadCanvas();
      server.send(JSON.stringify({ type: "state", canvas: canvas2 }));
    ...
    if (request.method === "PUT") {
      const newCanvas = await request.json();
      if (newCanvas.shapes)
        canvas.shapes = newCanvas.shapes;
      if (newCanvas.viewport)
        canvas.viewport = newCanvas.viewport;
...
function jitter(x, y, amount, rand) {
  return {
    x: x + (rand() - 0.5) * amount,
    y: y + (rand() - 0.5) * amount
  };
...
function roughLine(x1, y1, x2, y2, rand, roughness = 1) {
...
__name(roughLine, "roughLine");
function roughRect(x, y, w, h, rand, roughness = 1) {
  const paths = [];
...
__name(roughRect, "roughRect");
function roughEllipse(cx, cy, rx, ry, rand, roughness = 1) {
  const points = [];
  const steps = 24;
...
__name(roughEllipse, "roughEllipse");
function roughDiamond(x, y, w, h, rand, roughness = 1) {
  const cx = x + w / 2;
  const cy = y + h / 2;
  const paths = [];
...
__name(roughDiamond, "roughDiamond");
function arrowHead(x, y, angle, size = 15) {
...
__name(arrowHead, "arrowHead");
function renderShape(shape, rand, roughness = 1) {
...
__name(renderShape, "renderShape");
function escapeXml(str) {
  return str.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&apos;");
}
__name(escapeXml, "escapeXml");
function renderToSvg(state, options = {}) {
...
__name(renderToSvg, "renderToSvg");
function renderToSvgHtml(state, options = {}) {
  const svg = renderToSvg(state, options);
  return svg.replace('<?xml version="1.0" encoding="UTF-8"?>\n', "");
}
__name(renderToSvgHtml, "renderToSvgHtml");

// src/viewer.ts
function renderViewer(canvasId, wsUrl) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
__name(renderViewer, "renderViewer");
...
app.get("/canvas/:id/export/excalidraw", async (c) => {
  ...
  const excalidraw = {
    ...
    elements: state.shapes.map((shape) => ({
      ...
      x: shape.x,
      y: shape.y,
      width: "width" in shape ? shape.width : void 0,
      height: "height" in shape ? shape.height : void 0,
      points: "points" in shape ? shape.points.map((p) => [p.x, p.y]) : void 0,
      ...
      text: "text" in shape ? shape.text : "label" in shape ? shape.label : void 0,
...
// .wrangler/tmp/bundle-4DkXLs/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4DkXLs/middleware-loader.entry.ts
